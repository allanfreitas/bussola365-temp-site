    ...
    switch (action) {
      case "webhook-ingestion":
        ...
        return NextResponse.json({
          message: "Webhook Ingestion - Processamento Concluido",
        });

      case "process-message":
        await messageRouterService.execute(paramId);
        return NextResponse.json({
          message: "Router Service - Processamento concluído",
        });

      case "process-transaction":
        await transactionBuilderService.execute(paramId);
        return NextResponse.json({
          message: "Transaction Builder - Processamento concluído",
        });
      ...
      case "answer-message":
        ...
        if (!transaction)
          return NextResponse.json(
            { error: "Transaction not found for message" },
            { status: 404 }
          );
        ...
        await responseService.execute(
          messageId,
          transaction.id,
          transaction.walletId
        );
        return NextResponse.json({
          message: "Answer Message - Processamento concluído",
        });
